...
def parse_restaurant(name, url):
    ...
    for day_div in day_divs:
        ...
        if month is not None:
            ...
            # If we couldn't find a month, we'll just try to use the previous date and hope that one was better
        ...
        for tr in tablerows:
            ...
            if menu_color is None:
                logging.warning(name + " - Failed to get a color for the menu: '" + menu_name + "' (" + str(date) + ")")
                menu_color = '#ffffff'
            # Sometimes there is no menu name, but just an image (e.g., for "Veggiedag")
            if not menu_name:
                img = sel_img(tds[0])
                img = img[0] if img else None
                menu_naam = 'Veggiedag' if is_veggiedag_img(img) else 'Menu'
